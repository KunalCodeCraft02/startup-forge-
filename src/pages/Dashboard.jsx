  ...
  return (
    <div className="mx-auto max-w-7xl px-4 sm:px-6 py-10">
      ...
      <div className="grid grid-cols-1 lg:grid-cols-[240px_1fr] gap-6">
        <aside className="glass rounded-2xl p-2 h-fit">
          {TABS.map((t) => (
            <button key={t.key} onClick={() => setTab(t.key)}
              className={`w-full flex items-center gap-2.5 rounded-xl px-3 py-2.5 text-sm text-left font-medium transition-all duration-200 ${tab === t.key ? "bg-violet-100/60 text-violet-700 shadow-sm" : "text-slate-400 hover:text-slate-700 hover:bg-white/40"}`}>
              <t.icon className="h-4 w-4" /> {t.label}
            </button>
          ))}
        </aside>
        <div className="space-y-4">
          {tab === "saved" && (saved.length ? <div className="grid grid-cols-1 md:grid-cols-2 xl:grid-cols-3 gap-5">{saved.map((idea) => <OpportunityCard key={idea.id} idea={idea} />)}</div> : <Empty title="No saved ideas yet" cta={<Link to="/explore" className="btn-primary text-sm">Explore opportunities</Link>} />)}
          ...
          {tab === "email" && (<div className="glass rounded-2xl p-6 max-w-2xl"><h3 className="text-slate-900 font-bold">Daily Opportunity Feed</h3><p className="text-sm text-slate-500 mt-1.5 leading-relaxed">A daily email with top ideas, trending industries, and research papers.</p><div className="mt-4 flex items-center justify-between rounded-xl bg-violet-50/40 border border-violet-200/30 p-3.5"><div><div className="text-sm text-slate-800 font-medium">Subscription status</div><div className="text-xs text-slate-400">{subscribed ? "Subscribed" : "Not subscribed"}</div></div><button onClick={() => { setSubscribed(!subscribed); toast.success(!subscribed ? "Subscribed" : "Unsubscribed"); }} className={`btn-ghost text-sm ${subscribed ? "bg-emerald-50/60 border-emerald-300/40 text-emerald-600" : ""}`}>{subscribed ? "Unsubscribe" : "Subscribe"}</button></div><button onClick={() => toast.success("Preview sent (mock)")} className="btn-primary text-sm mt-4"><FiDownload className="h-4 w-4" /> Send me a preview</button></div>)}
        </div>
      </div>
    </div>
  );
