...
export default function SearchBar({ large = false, defaultValue = "" }) {
  ...
  return (
    <form onSubmit={submit} className={`relative ${large ? "max-w-3xl" : "max-w-xl"} w-full mx-auto group`}>
      ...
      <div className="relative flex items-center gap-2 liquid-glass rounded-2xl p-2 pl-5">
        ...
        <input
          ...
          className={`flex-1 bg-transparent outline-none placeholder:text-slate-400/70 text-slate-800 font-medium ${large ? "py-3 text-base sm:text-lg" : "py-2 text-sm"}`}
